...
for (n in c(100, 500, 2000)){
  ...
  sett_res = data.frame(sim = paste(sim_seed, 1:reps, sep = "-"), censoring, n, distX, perc_censored = NA, 
                        ...
                        alpha_expo = NA, beta_expo = NA, gamma_expo = NA, 
                        se_alpha_expo = NA, se_beta_expo = NA, se_gamma_expo = NA, 
                        alpha_drop = NA, beta_drop = NA, gamma_drop = NA, 
                        se_alpha_drop = NA, se_beta_drop = NA, se_gamma_drop = NA, 
                        time_weibull = NA, time_expo = NA, time_drop = NA)
  ...
  for (r in 1:reps) {
    ...
    # Methods 2-4: Extrapolated CMI using different extrapolation methods
    for (extrap_meth in c("w", "e", "d")) {
      ## Define column names based on extrap_method
      if (extrap_meth == "e") {
        est_cols = c("alpha_expo", "beta_expo", "gamma_expo")
        se_cols = c("se_alpha_expo", "se_beta_expo", "se_gamma_expo")
      } else if (extrap_meth == "w") {
        est_cols = c("alpha_weibull", "beta_weibull", "gamma_weibull")
        se_cols = c("se_alpha_weibull", "se_beta_weibull", "se_gamma_weibull")
      } else {
        est_cols = c("alpha_drop", "beta_drop", "gamma_drop")
        se_cols = c("se_alpha_drop", "se_beta_drop", "se_gamma_drop")
      }
      ...
      imp_dat = cmi_sp(imputation_model = Surv(time = w, event = d) ~ z,
                       ...
                       surv_beyond = extrap_meth) ## and extrapolated using extrap_method
      ...
      if (imp_dat$code) {
        if (B == 0) {
          ...
          sett_res[r, est_cols] = pooled_est
          ...
          sett_res[r, se_cols] = sqrt(pooled_var)
        }
      }
    }
