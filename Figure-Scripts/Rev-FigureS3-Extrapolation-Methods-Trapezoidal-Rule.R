...
res = read.csv(file = "https://raw.githubusercontent.com/sarahlotspeich/ExtrapolationBeforeImputation/main/Figure-Data/rev_data_figureS3.csv")
...
ggsave("Rev_FigureS3.png", width = 10, height = 10, units = "in")

## Note: 20 rows will be removed because beta is NA 
beta_res |> 
  group_by(extrap, censoring, n) |> 
  summarize(reps_na = sum(is.na(beta))) |> 
  arrange(desc(reps_na))
## There were <= 4 instances per setting (<=0.4%)