...
for (censoring in c("light", "heavy", "extra_heavy")) {
  # And different sample sizes n = 100, 500, 2000
  for (n in c(100, 500, 2000)){
    ...
    for (r in 1:reps) {
      ...
      # Method 2: Extrapolated CMI (B = 20 imputations via bootstrapping)
      fit_extrap = cmi_sp_bootstrap(imputation_model = Surv(time = w, event = d) ~ z, ## imputation model specification
      ...
      fit_non_extrap = cmi_sp_bootstrap(imputation_model = Surv(time = w, event = d) ~ z, ## imputation model specification
                                        analysis_model = y ~ imp + z, ## analysis model specification 
                                        data = dat, ## dataset
                                        ...
                                        surv_beyond = "w", ## and extrapolated using the Weibull extension
                                        B = 20) ## number of imputations
      sett_res[r, c("alpha_non_extrap", "beta_non_extrap", "gamma_non_extrap")] = fit_non_extrap$Est
      sett_res[r, c("se_alpha_non_extrap", "se_beta_non_extrap", "se_gamma_non_extrap")] = fit_non_extrap$SE
      
      # Save results
      write.csv(x = sett_res, 
                file = paste0("Table1-Rev/", censoring, "_n", n, "_seed", sim_seed, ".csv"), 
                row.names = F)
    }
  }
}
